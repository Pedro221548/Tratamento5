  ...
  groupedByPerson.forEach((personRecords, personName) => {
    ...
    const finalizeShift = (punches: RawRecord[]) => {
        ...
        const findVal = (keywords: string[]) => {
          // Finds a column header that contains one of the keywords
          const key = keys.find(k => keywords.some(kw => k.toLowerCase().includes(kw)));
          return key ? String(originalRow[key]).trim() : '';
        };

        // Enhanced CPF/ID detection with more common terms from Brazilian timesheet systems
        const cpf = findVal([
            'cpf', 
            'documento', 
            'doc.', 
  ...
  jsonData.forEach((row) => {
    ...
    keys.forEach(key => {
        ...
        if (
            ...
            headerLower.includes('cpf') || // Ignore CPF column as date
            headerLower.includes('doc') || // Ignore Doc column as date
            headerLower.includes('matr')   // Ignore Matricula as date
        ) {
            return;
        }
